...
Usage::
    ...
    syn_games_client = GamesAPIClient(url=simpl_url, auth=simpl_auth)
...
Retrieves all resources (essentially a simple ``GET`` on the endpoint)::
    ...
    syn_games_client.runusers.all()  # GET /runusers/

``.filter(**kwargs)`` calls a ``GET`` with ``kwargs`` as querystring values::

    syn_games_client.runusers.filter(run=12, world=1)  # GET /runusers/?run=12&world=1
...
::
    ...
    syn_games_client.runusers.filter(run=12, world=1)  # GET /runusers/?run=12&world=1
    syn_games_client.runusers.filter(id=12)  # GET /runusers/12/
    syn_games_client.users.filter(username='alice')  # GET /users/alice/
...
returning a new ``Resource``::
    ...
    runuser = syn_games_client.runusers.create({'run': 12, 'world': 1})  # POST /runusers/
...
to create the resource.
    ...
    run = syn_games_client.runs.get_or_create(game=12, defaults={'active': True})  # GET /runs/?game=12, then POST /runs/
...
it will call ``.create``::
    ...
    runuser = syn_games_client.runusers.create_or_update({'id': 1234, 'world': 1})  # PUT /runusers/1234/
...
Will issue a ``DELETE``, and will use ``pk`` as part of the URL::
    ...
    syn_games_client.runusers.delete(24)  # DELETE /runusers/24/
...
::
    ...
    syn_games_client.bulk.results.create([...], return_ids=False)
    syn_games_client.bulk.results.delete(**lookup)
...
::
    ...
    syn_games_client.scenario(id=123).rewind() 
